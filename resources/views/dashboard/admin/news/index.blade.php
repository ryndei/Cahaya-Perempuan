    ...
    <div class="mb-4 flex items-center justify-between">
      <form method="GET" class="flex gap-2">
        <input type="text" name="q" value="{{ old('q', $searchQuery) }}"
               placeholder="Cari judul/excerpt/isi…" class="border rounded-lg px-2 py-2 text-sm w-64">
        <select name="status" class="border rounded-lg text-sm">
      ...
      <a href="{{ route('admin.news.create') }}"
         class="px-3 py-2 rounded-lg bg-purple-700 text-white text-sm">+ Tambah Berita</a>
    </div>
          ...
          @forelse($items as $it)
            <tr>
              <td class="px-4 py-3">
                ...
                <div class="text-slate-500 text-xs line-clamp-1 max-w-[52ch]">{{ $it->excerpt }}</div>
              </td>
              <td class="px-4 py-3">
